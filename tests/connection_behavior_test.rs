...
fn test_cloudflare_connection_behavior_detection() {
    ...
    headers.insert("server-timing".to_string(), "cfRequestDuration;dur=45.2".to_string());
    ...
    assert!(!evidence.is_empty(), "Should detect CloudFlare connection behavior");

    let cf_evidence = evidence.iter()
        .find(|e| e.signature_matched == "connection-behavior-cloudflare");
    assert!(cf_evidence.is_some(), "Should have CloudFlare-specific evidence");

    let ev = cf_evidence.unwrap();
    assert!(ev.confidence >= 0.70, "CloudFlare detection should have high confidence");
    assert!(ev.description.contains("CloudFlare"), "Description should mention CloudFlare");
}
...
fn test_aws_cloudfront_via_header_detection() {
    ...
    let mut headers = HashMap::new();
    headers.insert("via".to_string(), "1.1 abc123.cloudfront.net (CloudFront)".to_string());
    headers.insert("connection".to_string(), "keep-alive".to_string());
    ...
    assert!(!evidence.is_empty(), "Should detect AWS CloudFront connection behavior");

    let aws_evidence = evidence.iter()
    ...
    assert!(ev.confidence >= 0.70, "AWS detection should have high confidence");
    assert!(ev.description.contains("AWS"), "Description should mention AWS");
}
...
fn test_akamai_via_header_detection() {
    ...
    let mut headers = HashMap::new();
    headers.insert("via".to_string(), "1.1 akamai.net (ghost) (AkamaiGHost)".to_string());
    headers.insert("connection".to_string(), "keep-alive".to_string());

    let evidence = analyzer.analyze_response_headers(&headers);

    assert!(!evidence.is_empty(), "Should detect Akamai connection behavior");

    let akamai_evidence = evidence.iter()
        .find(|e| e.signature_matched == "connection-behavior-akamai");
    assert!(akamai_evidence.is_some(), "Should have Akamai-specific evidence");
}
...
fn test_multiple_provider_patterns_in_headers() {
    ...
    headers.insert("server-timing".to_string(), "cdn-upstream;dur=5".to_string());

    let evidence = analyzer.analyze_response_headers(&headers);

    // Should only match AWS due to cloudfront via header
    let aws_matches = evidence.iter()
...
fn test_no_false_positives_on_generic_headers() {
    ...
    for ev in &evidence {
        assert!(ev.confidence < 0.70 || evidence.is_empty(),
...
fn test_azure_via_detection() {
    ...
    assert!(!evidence.is_empty(), "Should detect Azure connection behavior");

    let azure_evidence = evidence.iter()
        .find(|e| e.signature_matched == "connection-behavior-azure");
    assert!(azure_evidence.is_some(), "Should have Azure-specific evidence");
}
...
fn test_case_insensitive_header_names() {
    ...
    headers.insert("Server-Timing".to_string(), "cfRequestDuration;dur=50".to_string());
    headers.insert("ALT-SVC".to_string(), "h3=\":443\"".to_string());
    headers.insert("Connection".to_string(), "keep-alive".to_string());

    let evidence = analyzer.analyze_response_headers(&headers);

    assert!(!evidence.is_empty(), "Case-insensitive matching should work");

    let cf_evidence = evidence.iter()
        .find(|e| e.signature_matched == "connection-behavior-cloudflare");
    assert!(cf_evidence.is_some(), "Should detect CloudFlare regardless of header case");
}
